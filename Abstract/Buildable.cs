...
namespace FirstTest
{
    ...
    public abstract class Buildable: NavigationMenu
    {
        ...
        public void Develop(string resource, int number) {
            GoTo(menu, act);
            OpenDetails(resource);
            
            int maximum = int.Parse(MyDriver.FindElement(Program.settings.Supplies.TechnologyDetails.BuildAmount).GetAttribute("max"));
